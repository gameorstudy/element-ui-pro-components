<template>
  ...
  >
    ...
    <template v-if="submitter" #footer>
      ...
      <!-- start -->
      <template v-else>
        <div class="dialog-form-footer" style="display: flex; justify-content: center;">
          <el-button v-bind="submitterProps.cancelButtonProps" @click="close" >{{ submitterProps.searchConfig.cancelText }}</el-button>
          <el-button type="primary" v-bind="submitterProps.confirmButtonProps" @click="submit">{{ submitterProps.searchConfig.confirmText }}</el-button>
        </div>
      </template>
      <!-- end -->
    </template>
  ...
  export default {
    ...
    computed: {
      ...
      initializedDialogProps() {
        ...
        return {
          ...dialogProps,
          'custom-class': `${dialogProps['custom-class'] ?? ''} dialog-form`,
          visible: undefined, // 使用全局的 visible / v-model
          title,
          width,
        }
      },
      ...
      submitterProps() {
        ...
        if (typeof this.submitter === 'object') {
          const { searchConfig = {}, confirmButtonProps = {}, cancelButtonProps = {} } = this.submitter
