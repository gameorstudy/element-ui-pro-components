...
> 可编辑表格 EditableTable 主要用于在表格内直接编辑的场景。它和之前的 ProTable、ProForm 不同，主要是在于它更多适合需要更灵活的设置，或许变得受控它才能发挥更大的作用，当然把它当成黑盒子也未尝不可，所以我还是跟随 Antd pro components 设计的脚步，支持受控和非受控两种模式。基于开发水平的原因，简化和删除了一些 API。
...
> 如果新建一行按钮位置设置为 `top`，因为设计原因表头和表格分离了，因此下面所有与表格相关的属性都不能修改表头的操作，比如列选择功能将失效，替代方法是自定义新建按钮，且设置 `recordCreatorProps` 为 `false` 即可。
...
| initialValues | 表单行数据默认值，组件初始渲染绑定 | `object` | - |
##### EditableTable Methods
| 属性 | 描述 | 类型 | 默认值 |
| ----------- | ----------- | ----------- | ----------- |
| getFormRef | 可编辑表格内 el-form 组件引用 | `() => Ref` | - |
| getTableRef | 可编辑表格内 el-table 组件引用 | `() => Ref` | - |
| getRowData | 非受控模式下获取行数据，可以传入第几行的数据，也可以传入 `rowKey` 的值 | `(rowIndex: string \| number) => T \| undefined` | - |
| getRowsData | 非受控模式下获取整个 table 的数据 | `() => T[] \| undefined` | - |
| setRowData | 非受控模式设置一行的数据，会将数据进行对象的合并，支持传入行号，也可以传入 `rowKey` 的值 | `(rowIndex: string \| number, data: Partial<T>) => void` | - |
##### RecordCreatorProps
...
| 属性 | 描述 | 类型 | 默认值 |
| ----------- | ----------- | ----------- | ----------- |
| type | 可编辑表格的类型，单行编辑或者多行编辑。`single` 表示只能同时新增一行和只能同时编辑一行，`multiple` 则开启实时保存。 |	`single` \| `multiple` | `single` |
...
| initialValue | 默认值，新建一行数据时会传入 | `any` | - |
