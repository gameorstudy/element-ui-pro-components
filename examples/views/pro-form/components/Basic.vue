...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onDelete(recordKey) {
        const formRef = this.$refs.proFormRef
        const { items } = formRef.getForm()
        console.log('items', items)
        formRef.setFieldValue('items',
          items.filter(item => item.id !== recordKey)
        )
      }
    }
  }
